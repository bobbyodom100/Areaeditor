...
IMPLEMENT_DYNCREATE(CAreaView, CFormView)

CAreaView::CAreaView()
	: CFormView(CAreaView::IDD)
{
...
void CAreaView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_AREA_TABS, m_ctlTabControl);
	//}}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(CAreaView, CFormView)
	//{{AFX_MSG_MAP(CAreaView)
	ON_NOTIFY(TCN_SELCHANGE, IDC_AREA_TABS, OnSelchangeAreaTabs)
	ON_NOTIFY(TCN_SELCHANGING, IDC_AREA_TABS, OnSelchangingAreaTabs)
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CAreaView::OnSelchangeAreaTabs(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  m_pDoc->m_iCurrentAreaTab = sel_index;
...
void CAreaView::OnSelchangingAreaTabs(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CAreaView::OnSize(UINT nType, int cx, int cy) 
{
...
void CAreaView::ShowPages (CAreaEditorDoc * pDoc)
  {
  m_pDoc = pDoc;

  m_ctlTabControl.SetCurSel (m_pDoc->m_iCurrentAreaTab);

	OnSelchangeAreaTabs(NULL, NULL);
  }
...
// FALSE otherwise.
// --------------------------------------------------------------------------
BOOL CAreaView::HandleKeyDownMsg(MSG* pMsg)
{
...
#define AddFlag(dlg, name, flag, flagarray)  \
  ...
  dlg->m_pFlags = &m_area->flag;   \
  dlg->m_pFlagsArray = &flagarray;  \
  dlg->m_pDoc = pDoc;


void CAreaView::CreatePages()
{
...
// Warning - if you move the summary page, update SUMMARY_PAGE define
  ...
  AddPage (namedlg,       "Name",             CAreaNameView,   IDD_AREA_NAME);
  AddPage (vnumsdlg,      "Vnums",            CAreaVnumsView,  IDD_AREA_VNUMS);

  if (pDoc->m_AreaType == eSMAUG)
    {
    AddPage (rangesdlg,     "Ranges / Economy", CAreaRangesView, IDD_AREA_RANGES);
    AddFlag (flagsdlg,      "Flags",            flags,    AreaFlags);
    AddPage (climatedlg,    "Climate",          CAreaClimateView,  IDD_AREA_CLIMATE);
    AddPage (neighboursdlg, "Neighbours",       CAreaNeighbourView,  IDD_AREA_NEIGHBOUR);
    }
 
  namedlg->m_pDoc = pDoc;
  namedlg->m_pLHview = m_pLHview;
