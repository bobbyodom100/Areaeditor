...
void CRoomView::OnSelchangeObjectTabs(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  m_pDoc->m_iCurrentRoomTab = sel_index;
...
void CRoomView::OnSelchangingObjectTabs(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CRoomView::ShowPages (CAreaEditorDoc * pDoc)
  {
  m_pDoc = pDoc;

  m_ctlTabControl.SetCurSel (m_pDoc->m_iCurrentRoomTab);
...
// FALSE otherwise.
// --------------------------------------------------------------------------
BOOL CRoomView::HandleKeyDownMsg(MSG* pMsg)
{
...
#define AddFlag(dlg, name, flag, flagarray)  \
  ...
  dlg->m_pFlags = &m_room->flag;   \
  dlg->m_pFlagsArray = &flagarray;  \
  dlg->m_pDoc = pDoc;


void CRoomView::CreatePages()
{
...
// Warning - if you move the summary page, update SUMMARY_PAGE define
  ...
  AddPage (summarydlg,    "Summary", CRoomSummaryPage,       IDD_ROOM_SUMMARY);
  AddPage (namedlg,       "Name",   CRoomNamePage,          IDD_ROOM_NAME);
  AddFlag (flagsdlg,      "Flags",  room_flags, RoomFlags);
