...
// MobileView.cpp : implementation file
//
#include "pch.h"
...
IMPLEMENT_DYNCREATE(CMobileView, CFormView)

CMobileView::CMobileView()
	: CFormView(CMobileView::IDD)
{
	//{{AFX_DATA_INIT(CMobileView)
	//}}AFX_DATA_INIT
...
void CMobileView::DoDataExchange(CDataExchange* pDX)
{
	CFormView::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CMobileView)
	DDX_Control(pDX, IDC_MOBILE_TABS, m_ctlTabControl);
	//}}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(CMobileView, CFormView)
	//{{AFX_MSG_MAP(CMobileView)
	ON_NOTIFY(TCN_SELCHANGE, IDC_MOBILE_TABS, OnSelchangeMobileTabs)
	ON_NOTIFY(TCN_SELCHANGING, IDC_MOBILE_TABS, OnSelchangingMobileTabs)
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CMobileView diagnostics

#ifdef _DEBUG
void CMobileView::AssertValid() const
{
	CFormView::AssertValid();
}

void CMobileView::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CMobileView message handlers


void CMobileView::OnSelchangeMobileTabs(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  m_pDoc->m_iCurrentMobileTab = sel_index;
...
void CMobileView::OnSelchangingMobileTabs(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CMobileView::OnSize(UINT nType, int cx, int cy) 
{
...
void CMobileView::ShowPages (CAreaEditorDoc * pDoc)
  {
  m_pDoc = pDoc;

  m_ctlTabControl.SetCurSel (m_pDoc->m_iCurrentMobileTab);

	OnSelchangeMobileTabs(NULL, NULL);
  }
...
// FALSE otherwise.
// --------------------------------------------------------------------------
BOOL CMobileView::HandleKeyDownMsg(MSG* pMsg)
{
...
#define AddFlag(dlg, name, flag, flagarray)  \
  ...
  dlg->m_pFlags = &m_mob->flag;   \
  dlg->m_pFlagsArray = &flagarray;  \
  dlg->m_pDoc = pDoc;  

void CMobileView::CreatePages()
{
...
// Warning - if you move the summary page, update SUMMARY_PAGE define
  ...
  AddPage (summarydlg,  "Summary", CMobileSummaryPage,   IDD_MOBILE_SUMMARY);
  AddPage (namedlg,     "Name",    CMobileNamePage,      IDD_MOBILE_NAME);
  AddPage (statsdlg,    "Stats",   CMobileStatsPage,     IDD_MOBILE_STATS);
  AddFlag (actdlg,      "Actions",      act, MobActFlags);
  AddFlag (affectdlg,   "Affected by",  affected_by, MobAffectFlags);
  AddFlag (attackdlg,   "Attacks",      attacks, MobAttackFlags);

  // ROM has form but not defenses
  if (m_mob->m_pDoc->m_AreaType == eROM)
    {   // need to use braces because AddFlag is a #define
    AddFlag (formdlg,     "Form",         form,     MobFormFlags);
    }
  else
    {
    AddFlag (defensedlg,  "Defenses",     defenses, MobDefenseFlags);
    }

  AddFlag (bodypartsdlg,"Body parts",   xflags,     MobPartsFlags);
  AddFlag (resistantdlg,"Resistant",    resistant,  MobResistFlags);
  AddFlag (immunedlg,   "Immmune",      immune,     MobResistFlags);
  AddFlag (susceptdlg,  "Susceptible",  susceptible,MobResistFlags);

  // Only SMAUG mobs speak
  if (m_mob->m_pDoc->m_AreaType == eSMAUG)
    {
    AddFlag (speaksdlg,   "Speaks",       speaks,     MobLanguageFlags);
    AddFlag (speakingdlg, "Speaking",     speaking,   MobLanguageFlags);
    }

  AddPage (resetsdlg,   "References", CMobileResetsPage, IDD_MOBILE_RESETS);
